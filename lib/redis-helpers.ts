  ...
  private static initializeConnection(): RedisClient | null {
    try {
      ...
      if (httpUrl && token && httpUrl.startsWith("https://")) {
        console.log(
          "[v0] Initializing HTTP-based Redis client (Upstash/Vercel KV)",
        );
      ...
      const tcpUrl = process.env.REDIS_URL || process.env.REDIS_TCP_URL;
      ...
      console.warn(
        "[v0] No valid Redis configuration found, running without cache",
      );
      console.warn(
        "[v0] For HTTP Redis (Upstash/Vercel KV): set KV_REST_API_URL and KV_REST_API_TOKEN",
      );
      console.warn(
        "[v0] For local TCP Redis: set REDIS_URL (default: redis://localhost:6379)",
      );
      return null;
    } catch (error) {
