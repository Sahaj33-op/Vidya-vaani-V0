...
class RAGPipeline:
    ...
    def _generate_response(
        self, question: str, search_results: List[Dict[str, Any]], language: str
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "retrieved_chunks": [
                r["text"][:200] + "..." for r in search_results[:2]
            ],  # Preview chunks
        }

    def _template_based_generation(
        self, question: str, context_chunks: List[str], language: str
    ) -> str:
        """
        ...
        if "fee" in question.lower() or "cost" in question.lower() or "फीस" in question:
            ...
            "admission" in question.lower()
            or "apply" in question.lower()
            or "प्रवेश" in question
        ):
            if any(
                "admission" in chunk.lower() or "application" in chunk.lower()
                for chunk in context_chunks
            ):
                return f"Here's the admission information:\n\n{context[:500]}...\n\nFor detailed guidance, please visit the admissions office."

        elif (
            "timetable" in question.lower()
            ...
            "contact" in question.lower()
            or "phone" in question.lower()
    ...
    def _extract_fee_info(self, context: str) -> str:
        ...
        fee_lines = [
            line
            for line in lines
            if "₹" in line or "fee" in line.lower() or "cost" in line.lower()
        ]
        return (
            "\n".join(fee_lines[:5])
            if fee_lines
            else "Fee information is available in our documents."
        )
