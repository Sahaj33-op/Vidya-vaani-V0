...
class ActionGetFeesInfo(Action):
    ...
    def _get_fee_structure(
        self, course_type: Text = None, department: Text = None
    ) -> Text:
...
class ActionDetectLanguage(Action):
    ...
    def run(
        ...
        return [
            {"event": "slot", "name": "language_preference", "value": detected_lang}
        ]
    ...
    def run(
        ...
        if last_bot_message and lang_pref and lang_pref != "en":
            ...
            dispatcher.utter_message(
                text=f"[Translated to {lang_pref}]: {translated_message}"
            )
...
class ValidateCourseInfoForm(FormValidationAction):
    ...
    def validate_course_type(
        ...
        if slot_value and slot_value.lower() in valid_courses:
            ...
            dispatcher.utter_message(
                text="Please specify: undergraduate, postgraduate, diploma, or certificate"
            )
    ...
    def validate_department(
        ...
        if slot_value and len(slot_value) > 2:
            ...
            dispatcher.utter_message(
                text="Please specify your department (e.g., Computer Science, Mechanical Engineering)"
            )
